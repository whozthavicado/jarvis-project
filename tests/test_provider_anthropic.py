"""AnthropicProvider tests — a fake Anthropic client built from the real
SDK's own event/message types, so the fake can't silently drift from the
real shape. No network or credentials required.
"""
import pytest
...
@pytest.mark.asyncio
async def test_request_uses_configured_model_effort_and_wraps_system_prompt():
    fake = _FakeAnthropicClient([], _final_message("ok"))
    provider = AnthropicProvider(
        model="claude-sonnet-5", max_tokens=8000, effort="medium", client=fake
    )
    ...
    # system_prompt is wrapped into a single cached content block here —
    # AnthropicProvider owns this formatting, callers pass plain text.
    assert kwargs["system"] == [
        {"type": "text", "text": "be helpful", "cache_control": {"type": "ephemeral"}}
    ]
    # ChatMessage -> plain role/content dicts
